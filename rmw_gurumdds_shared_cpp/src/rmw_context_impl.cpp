...
rmw_ret_t
rmw_context_impl_t::initialize_participant(const char * node_name, const char * node_namespace, const bool localhost_only)
{
  ...
  auto scope_exit_dp_finalize = rcpputils::make_scope_exit(
    [ctx, &publisher_qos, &subscriber_qos]() mutable
    {
  ...
  this->publisher = dds_DomainParticipant_create_publisher(this->participant, &publisher_qos, nullptr, 0);
  ...
  this->subscriber = dds_DomainParticipant_create_subscriber(this->participant, &subscriber_qos, nullptr, 0);
