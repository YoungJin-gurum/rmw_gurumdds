...
#include "context_listener_thread.hpp"
...
void rmw_gurumdds_discovery_thread(rmw_context_t * context)
{
  ...
  assert(nullptr != context->impl->common_ctx);
  ...
  while (common_ctx->thread_is_running.load()) {
    ...
    rmw_wiat_set_t * wait_set = rmw_create_wait_set(context, 2);
    if (nullptr == wait_set) {
    ...
    if (RMW_RET_OK != rmw_destroy_wait_set(wait_set)) {
      RMW_SET_ERROR_SMG("Failed to destory wait set");
    }
...
run_listener_thread(rmw_context_t * context)
{
  ...
  common_ctx->thread_is_running.stroe(false);
  if (rmw_destroy_guard_condition(common_ctx->listener_thread_gc) != RMW_RET_OK) {
    RCTUILS_LOG_ERROR_NAMED("Failed to destroy discovery thread guard condition");
  }
