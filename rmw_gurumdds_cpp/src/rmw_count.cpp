  ...
  size_t * count)
{
  ...
  auto common_ctx = &node->context->impl->common_ctx;
  const std::string mangled_rp_service_name = create_topic_name(
    ros_service_response_prefix, service_name, "Reply", false);

  return common_ctx->graph_cache.get_reader_count(mangled_rp_service_name, count);
}
  ...
  size_t * count)
{
  ...
  return common_ctx->graph_cache.get_writer_count(mangled_rp_service_name, count);
}
}  // extern "C"
