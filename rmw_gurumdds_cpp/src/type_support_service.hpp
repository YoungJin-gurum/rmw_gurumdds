...
inline void
ros_guid_to_dds_guid(int8_t * guid_ros, dds_GUID_t * guid_dds)
{
  auto prefix_size = sizeof(guid_dds->prefix);
  auto entityId_size = sizeof(guid_dds->entityId);
  memcpy(guid_dds->prefix, guid_ros, prefix_size);
  memcpy(&guid_dds->entityId, &guid_ros[prefix_size], entityId_size);
}
...
inline void
ros_sn_to_dds_sn(int64_t sn_ros, dds_SequenceNumber_t * sn_dds)
{
  sn_dds->high = ((sn_ros) & 0xFFFFFFFF00000000LL) >> 32;
  sn_dds->low = (sn_ros) & 0x00000000FFFFFFFFLL;
}
